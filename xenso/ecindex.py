"""ECindex: E and C index computation following Takahashi et al. 2011."""

import matplotlib.pyplot as plt
import numpy as np
import numpy.polynomial.polynomial as poly
import xarray as xr
from xeofs.single import EOF

from .core import compute_anomaly, compute_climatology, xconvolve
from .preprocessing import normalize_coords


class ECindex:
    """
    Computes the E and C index according to Takahashi et al. 2011.
    """

    def __init__(
        self,
        sst_data: xr.DataArray,
        isanomaly: bool = False,
        lat_range: tuple[float, float] | None = (-10, 10),
        long_range: tuple[float, float] | None = (110, 290),
        climatology: xr.DataArray | None = None,
        base_period: tuple[str, str] = ("1979-01-01", "2009-12-30"),
        corr_factor: list[int] | None = None,
        smooth_kernel: list[int] | None = None,
    ):
        self.sst_data = sst_data
        self.lat_range = lat_range
        self.long_range = long_range
        self.base_period = base_period
        if climatology is None and not isanomaly:
            climatology = compute_climatology(self.sst_data, base_period)
        self.climatology = climatology
        if not isanomaly:
            self.sst_data = compute_anomaly(self.sst_data, self.climatology)
        self._compute_pcs()
        self.smooth_kernel = smooth_kernel if smooth_kernel is not None else [1, 2, 1]
        if corr_factor is None:
            # initialize to neutral so _auto_corr_factor can read self.eofs
            self.corr_factor = [1, 1]
            self._auto_corr_factor()
        else:
            self.corr_factor = corr_factor

    def _compute_pcs(self) -> None:
        _subset = normalize_coords(self.sst_data).sel(
            lat=slice(*self.lat_range),  # type: ignore[misc]
            lon=slice(*self.long_range),  # type: ignore[misc]
        )
        self.model = EOF(n_modes=2, use_coslat=True)
        self.model.fit(_subset.sel(time=slice(*self.base_period)), dim="time")
        self._scale = 1 / np.sqrt(self.model.explained_variance())
        self.anom_pcs = self.model.transform(_subset) * self._scale
        self.anom_smooth_pcs = None

    def _corrected_pcs(self) -> xr.DataArray:
        return self.anom_pcs * self.corr_factor

    def _auto_corr_factor(self) -> None:
        _eofs = self.eofs
        _subset = dict(lat=slice(-5, 5), lon=slice(180, 200))
        new_corr_factor = np.zeros(2)
        new_corr_factor[0] = 1 if _eofs.sel(mode=1, **_subset).mean() > 0 else -1
        new_corr_factor[1] = 1 if _eofs.sel(mode=2, **_subset).mean() > 0 else -1
        self.corr_factor = new_corr_factor

    def _compute_index(self, smooth: bool = False) -> xr.Dataset:
        _pcs = self.pcs_smooth if smooth else self.pcs
        pc1 = _pcs.sel(mode=1)
        pc2 = _pcs.sel(mode=2)
        eindex = (pc1 - pc2) / (2 ** (1 / 2))
        eindex.name = "E_index"
        cindex = (pc1 + pc2) / (2 ** (1 / 2))
        cindex.name = "C_index"
        return xr.merge([eindex, cindex])

    @property
    def corr_factor(self) -> xr.DataArray:
        return self._corr_factor

    @corr_factor.setter
    def corr_factor(self, corr_factor: list[int]) -> None:
        self._corr_factor = xr.DataArray(
            np.array(corr_factor),
            coords=[("mode", [1, 2])],
        )

    @property
    def smooth_kernel(self) -> xr.DataArray:
        return self._smooth_kernel

    @smooth_kernel.setter
    def smooth_kernel(self, smooth_kernel: list) -> None:
        kernel = np.array(smooth_kernel)
        self._smooth_kernel = xr.DataArray(kernel / kernel.sum(), dims=["time"])

    @property
    def clim_pcs(self) -> xr.DataArray:
        return self.model.scores().T * self._scale * self.corr_factor

    @property
    def pcs(self) -> xr.DataArray:
        return self._corrected_pcs().T

    @property
    def pcs_smooth(self) -> xr.DataArray:
        if self.anom_smooth_pcs is None:
            self.anom_smooth_pcs = xconvolve(self.pcs, self._smooth_kernel, dim="time")
        return self.anom_smooth_pcs

    @property
    def ecindex(self) -> xr.Dataset:
        return self._compute_index()

    @property
    def ecindex_smooth(self) -> xr.Dataset:
        return self._compute_index(smooth=True)

    @property
    def eofs(self) -> xr.DataArray:
        return self.model.components() * self.corr_factor / np.sqrt(self.model.explained_variance())

    @property
    def patterns(self) -> xr.Dataset:
        ec_clim_period = xr.concat(
            [self.ecindex.E_index, self.ecindex.C_index],
            dim="mode",
        ).sel(time=slice(*self.base_period))
        cov = xr.cov(
            ec_clim_period,
            self.sst_data.sel(time=slice(*self.base_period)),
            dim="time",
        )
        std = ec_clim_period.std("time")
        reg = cov / std
        return xr.Dataset(
            data_vars=dict(
                E_pattern=(["lat", "lon"], reg.sel(mode=0).data),
                C_pattern=(["lat", "lon"], reg.sel(mode=1).data),
            ),
            coords={"lat": self.sst_data.lat, "lon": self.sst_data.lon},
            attrs=dict(description="E and C regression patterns"),
        )

    @staticmethod
    def compute_alpha(pc1, pc2, return_fit=False):
        """
        Compute the alpha parameter measuring non-linearity of the E and C index.
        """
        coefs = poly.polyfit(pc1, pc2, deg=2)
        if return_fit:
            xfit = np.arange(pc1.min(), pc1.max() + 0.1, 0.1)
            fit = poly.polyval(xfit, coefs)
            return coefs[-1], xfit, fit
        return coefs[-1]

    @staticmethod
    def plot_kiwi(pc1, pc2, ax: plt.Axes = None):  # pragma: no cover
        """
        Kiwi plot of PC1 vs PC2 for DJF season means.
        """
        if ax is None:
            _, ax = plt.subplots(figsize=(6, 6))

        pc1 = pc1.sel(time=pc1.time.dt.month.isin([12, 1, 2]))
        pc1 = pc1.resample(time="QS-DEC").mean().dropna("time")
        pc2 = pc2.sel(time=pc2.time.dt.month.isin([12, 1, 2]))
        pc2 = pc2.resample(time="QS-DEC").mean().dropna("time")

        alpha, xfit, fit = ECindex.compute_alpha(pc1, pc2, return_fit=True)

        ax.axhline(0, color="k", linestyle="--", alpha=0.2)
        ax.axvline(0, color="k", linestyle="--", alpha=0.2)
        x = np.linspace(-6, 6, 100)
        ax.plot(x, x, color="k", alpha=0.5, lw=1)
        ax.plot(-x, x, color="k", alpha=0.5, lw=1)
        ax.scatter(pc1, pc2, s=8, marker="o", c="w", edgecolors="k", linewidths=0.5)
        ax.plot(xfit, fit, c="r", label=f"$\\alpha=${alpha:.2f}")
        ax.set_xlabel("PC1")
        ax.set_ylabel("PC2")
        ax.set_xlim(-4, 6)
        ax.set_ylim(-6, 4)
        ax.legend()
